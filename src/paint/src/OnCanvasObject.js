...
import { setMousePosition, setObjectPosition } from "./functions.js";
...
class OnCanvasObject {
	...
	constructor(x, y, width, height, hideMainCanvasHandles) {
		...
		this.$el = $(E("div"))
			.addClass("on-canvas-object")
			.appendTo(PaintJSState.$canvas_area);
		if (this.hideMainCanvasHandles) {
			PaintJSState.canvas_handles.hide();
		}
		$(window).on(
			"resize theme-load",
			(this._global_resize_handler = () => {
				this.position();
			}),
		);
	}
	position(updateStatus) {
		...
		if (updateStatus) {
			...
			setMousePosition(true, this.x, this.y);
			setObjectPosition(true, this.width, this.height);
		}
	}
	destroy() {
		this.$el.remove();
