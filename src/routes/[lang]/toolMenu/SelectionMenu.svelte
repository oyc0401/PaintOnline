<script>
  ...
  import { quickChangeTool, clickMenu } from "$store/paintFunction.js";
  import "./menu.css";
  ...
  {#if menuState.showDropdown && menuState.dropdownId == MENU_NUMBER}
    <div class="dropdown-area small-dropdown menu-top">
      <button
        ...
        onclick={() => quickChangeTool(TOOL_SELECT, MENU_NUMBER)}
      >
        <SelectRectangleIcon />
        <p>사각형으로 선택</p>
      </button>
      <button
        ...
        onclick={() => quickChangeTool(TOOL_FREE_FORM_SELECT, MENU_NUMBER)}
      >
        <SelectFreeIcon />
        <p>자유형으로 선택</p>
      </button>
